...
# %%
# Parse dates
df["Date"] = df.apply(lambda x: parser.parse(x["Date"]), axis=1)
...
df.drop(["Experimental", "Minimum QGIS version", "Uploaded by"], axis=1, inplace=True)
...
# Plot downloads per time
fig, ax = plt.subplots(figsize=(10, 8))
plt.plot(df_version["Version"], df_version["Downloads_sum"], marker=".", markersize=10)
plt.grid()
...
# Plot downloads per time
fig, ax = plt.subplots(figsize=(10, 8))
plt.plot(df_version["Version"], df_version["Downloads"], marker=".", markersize=10)
