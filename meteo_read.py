...
print("Meteo", datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), sep=",", end=",", file=log_file)
...
for data in meteo_data_elements:
    ...
    date_time = datetime.datetime.strptime(data.find('tsValid_issued_UTC').text, '%d.%m.%Y %H:%M %Z')
