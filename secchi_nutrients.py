...
df["Date"] = pd.to_datetime(
    dict(year=df["Godina"], month=df["Mjesec"], day=df["Dan"])
) + pd.to_timedelta(df["Sat i minute (hh:mi)"])

# %%
# Drop date remaining data
df.drop(
    ["Godina", "Mjesec", "Dan", "Sat i minute (hh:mi)"], inplace=True, axis="columns"
)
# %%
# Select only Secchi depth and measurement attributes
df_secchi = df[df["Nutrient"] == "Secchi depth"].copy()
df_secchi.drop(
    ["Nutrient", "Mjerna jedinica", "Dubina uzorkovanja"], inplace=True, axis="columns"
)
