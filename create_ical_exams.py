...
lecturer = 'Oštir'
subjects = ['Programska orodja v geodeziji', 'Avtomatska obdelava podatkov', 'Daljinsko zaznavanje I', 'Avtomatska obdelava podatkov', 'Uporabno daljinsko zaznavanje', 'Analize prostorskih podatkov']

# Read list of exams and extract important columns
exams_list = pd.read_excel(exams_list_fn)
exams_list = exams_list[['Vrsta študija', 'letnik', 'Izvajalec', 'Predmet', 'Datum izpita', 'Datum izpita.1', 'Datum izpita.2', 'Dodaten rok']]
...
# Melt dataframe
exams_lecturer_m = pd.melt(exams_lecturer, id_vars=['Vrsta študija', 'letnik', 'Predmet'], value_name='Datum')
...
with open(exams_calendar, 'w') as f:
    ...
    for row in exams_lecturer_m.iterrows():
        ...
        print('DTSTART;VALUE=DATE:', str(row[1][3])[:10].replace('-', ''), sep='', file=f)
