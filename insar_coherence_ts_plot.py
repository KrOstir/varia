...
ts_df.drop(
    ["Unnamed: 0", "Obmocje", "Razred", "area", "slope", "aspect", "latitude", "HAB"],
    axis=1,
    inplace=True,
)
...
ts_df_id_pol_asc = ts_df_id_pol_asc.loc[
    ~ts_df_id_pol_asc.index.duplicated(keep="first")
]
ts_df_id_pol_des = ts_df_id_pol["DES"].dropna()
ts_df_id_pol_des = ts_df_id_pol_des.loc[
    ~ts_df_id_pol_des.index.duplicated(keep="first")
]
