...
def sicris_get_info(mstid):
    ...
    for field in bib_fields:
        bib_r.update(
            {field: float(bib_json["RECAPITUALITION"][0][field].replace(",", "."))}
        )
    return bib_r
...
# %%
# Add English fields
mentors_df["Field"] = mentors_df.apply(lambda x: research_field(x["Smer"]), axis=1)
# %%
# Add Sicris URL
mentors_df["Sicris_URL"] = mentors_df["Sicris"].apply(
    lambda x: "{}{}".format(sicris_url, str(x).zfill(5))
)
